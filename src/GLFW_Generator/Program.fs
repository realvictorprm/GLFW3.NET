...
let root_directory = System.IO.Directory.GetParent(Environment.CurrentDirectory).Parent.Parent.Parent.FullName;
...
type Generator() =
    ...
    let DeprecateGeneratedMethods (ctx:CppSharp.AST.ASTContext) =
        ctx.FindDecl<CppSharp.AST.Function>("GetRequiredInstanceExtensions") |> Seq.iter(fun i -> printfn "func: %A" i; if not(isNull(i)) then i.Access <- AST.AccessSpecifier.Internal; i.GenerationKind <- AST.GenerationKind.Internal)
        ()
